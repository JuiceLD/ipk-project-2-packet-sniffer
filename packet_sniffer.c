...
char* process_filter(int filter_flags, unsigned int port_value, char * filter_string)
{
    ...
    if ((filter_flags == 0 && port_flag == true) || (port_flag == true && udp_flag == false && tcp_flag == false))
    {
	...
	// Port and interface and other arguments that affect the filter were filled
	if (port_flag == true && filter_flags != 0)
	{
		if (arp_flag)
		{
			filter_flags--;
